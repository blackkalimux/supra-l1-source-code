...
#[utoipa::path(
    post,
    path = "/rpc/v1/tables/{table_handle}/item",
    operation_id = "table_item_by_key_v1",
    request_body( content = TableItemRequest,
                  description = "Request containing item key and value types and item key itself",
                  content_type = "application/json"
    ),
    responses(
        (status = 200, description = "Item of the table", content((Option<MoveValueApi> = "application/json")))
    ),
    params(
...
pub async fn table_item_by_key(
...
) -> crate::error::Result<Json<Option<MoveValue>>> {
    ...
    let key_tag = TypeTag::from_str(key_type.as_str())?;

    // Convert key to lookup version for DB
    let vm_key = converter
        .try_into_vm_value(&key_tag, key.clone().into())
        .map_err(|err| {
            crate::error::Error::MoveConverterError(format!("Invalid key type: {err}"))
        })?;
    let value_tag = TypeTag::from_str(value_type.as_str())?;
    state
        .move_store
        .read_table_item(table_handle.into_inner(), vm_key)?
        .map(|bytes| converter.convert_to_move_value(value_tag, &bytes))
        .transpose()
